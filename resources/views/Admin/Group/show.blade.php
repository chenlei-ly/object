...
<div class="mws-panel grid_8" id="box">
	<div class="mws-panel-header">
    	<span><i class="icon-table"></i>管理组列表</span>
    </div>
	            ...
	            @foreach($data as $v)
	            	<tr class="odd">
	                    ...
	                    <td class=" ">
                            ...
                            <a href="group/{{$v->id}}/edit">修改</a>
							<a href="/shouquan/{{$v->id}}">授权</a>
	                    </td>
	                </tr>
	            @endforeach
	            </tbody>
...
<script>
    //分页
    function page(num) {
    	$.get('/group',{'num':num},function(data){
    		//alert(data);
    		$('#box').html(data);
    	})
    }
    ...
    function del(id,newpage){
        $.get('/groupdel',{id:id},function(data){
            alert(id);
            if(data == 1){
                page(newpage);
            } else {
